(function () {
  ...
  let pendingChampionHighlight = false;
  ...
  function buildProfileCard(username) {
    ...
    panel.innerHTML = `
      ...
      <button class="account-tutorial-help tutorial-help-button" type="button" title="Show tutorial" aria-label="Show tutorial" aria-expanded="false">?</button>
  ...
  function renderHomeAccount() {
    ...
    if (username) {
      ...
      const tutorialHelpButton = profile.querySelector('.account-tutorial-help');
      if (tutorialHelpButton) {
        tutorialHelpButton.addEventListener('click', () => {
          document.getElementById('home-tutorial-help')?.click();
        });
      }
      if (logoutButton) {
  ...
  function openLeaderboard(options = {}) {
    ...
    if (options.showChampionAfterRender) {
      pendingChampionHighlight = true;
    }
    if (options.showChampion === true) {
      window.setTimeout(() => showRankChampion(activeRankType), 380);
    }
  }

  function setLeaderboardSpotlight(isActive) {
    const wrapper = document.getElementById('leaderboard-wrapper');
    if (!wrapper) return;
    wrapper.classList.toggle('completion-spotlight', Boolean(isActive));
  }
  ...
  async function fetchLeaderboard() {
    ...
    try {
      ...
      pendingChampionHighlight = false;
  ...
  function renderRank(type) {
    ...
    const rankSource = type === 'time'
      ? leaderboardData.filter(item => rankNumber(item.time) !== Infinity)
      : leaderboardData;

    if (!rankSource.length) {
      pendingChampionHighlight = false;
      listArea.innerHTML = '<p class="rank-empty">No ranked times yet</p>';
      return;
    }

    const sorted = [...rankSource].sort((a, b) => {
    ...
    listArea.innerHTML = bestByUser.slice(0, 10).map((item, index) => {
      ...
      const value = type === 'steps'
        ...
        : `${rankNumber(item.time) === Infinity ? '-' : `${escapeHtml(item.time)}s`}`;
      return `
    ...
    const wrapper = document.getElementById('leaderboard-wrapper');
    if (pendingChampionHighlight && wrapper?.classList.contains('expanded')) {
      pendingChampionHighlight = false;
  ...
  function showRankChampion(type) {
    ...
    const rankSource = type === 'time'
      ? leaderboardData.filter(item => rankNumber(item.time) !== Infinity)
      : leaderboardData;
    const champion = [...rankSource].sort((a, b) => {
      const primary = rankNumber(a[type]) - rankNumber(b[type]);
  ...
  function savePlacementRecord(record) {
    ...
    const payload = {
      ...
      time: record.timeRankable === false ? '' : Number(record.time || 0),
      timeRankable: record.timeRankable !== false,
    ...
    return fetch(SCRIPT_URL, {
      ...
      return new Promise(resolve => {
        setTimeout(() => {
          fetchLeaderboard().finally(() => resolve(true));
        }, 1200);
      });
    }).catch(() => false);
  }

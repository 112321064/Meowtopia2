document.addEventListener('DOMContentLoaded', function () {
  ...
  let isLevelOneProGuidePending = false;
  let isLevelOneSignalIntroPending = false;
  let hasCompletedLevelOneProIntro = false;
...
`, 50);
  ...
  const isLevelOneProTutorial = tutorialKey === 'place-cat';
  ...
  function syncActionButtonLabels() {
    [thresholdBtn, undoBtn, saveMapBtn, resetBtn].forEach(button => {
      if (!button) {
        return;
      }
  ...
  function updateProToolVisibility() {
    [thresholdBtn, saveMapBtn].forEach(button => {
      if (button) {
  ...
  function setLevelOneProGuideActive(isActive) {
    isLevelOneProGuidePending = isActive;
    document.body.classList.toggle('level-one-pro-guide-active', isActive);
    if (modeToggleBtn) {
      modeToggleBtn.classList.toggle('pro-mode-guide-pulse', isActive);
    }
  }

  function showLevelOneSignalIntro() {
    if (hasCompletedLevelOneProIntro || !isLevelOneProTutorial) {
      return;
    }

    isLevelOneSignalIntroPending = true;
    const signalModal = document.createElement('div');
    signalModal.className = 'tutorial-modal level-one-signal-modal show';
    signalModal.innerHTML = `
      <div class="tutorial-card signal-intro-card">
        ...
        <button class="tutorial-ok-button signal-intro-ok" type="button">OK!</button>
      </div>
    `;
    document.body.appendChild(signalModal);
    signalModal.querySelector('.signal-intro-ok')?.addEventListener('click', function () {
      hasCompletedLevelOneProIntro = true;
      isLevelOneSignalIntroPending = false;
      signalModal.remove();
      startMissionTimer();
    });
  }
  ...
  function renderGrid(size, layout = []) {
    ...
    for (let i = 0; i < size * size; i++) {
      ...
      cell.addEventListener('click', function () {
        if (isLevelOneProGuidePending || isLevelOneSignalIntroPending) {
          return;
        }
  ...
  tutorialOkButton?.addEventListener('click', function () {
    ...
    if (isLevelOneProTutorial && !hasCompletedLevelOneProIntro) {
      setLevelOneProGuideActive(true);
      return;
    }
    startMissionTimer();
  });
  ...
  modeToggleBtn?.addEventListener('click', function () {
    ...
    shouldAnimateFishOnNextUpdate = false;
    hasShownCompletionModal = false;
    const shouldShowSignalIntro = isLevelOneProGuidePending && !isGameMode && !hasCompletedLevelOneProIntro;
    if (shouldShowSignalIntro) {
      setLevelOneProGuideActive(false);
    }
    updateModeUI();
    applyProbabilitiesToGrid(probabilitiesCache, clickableBox);

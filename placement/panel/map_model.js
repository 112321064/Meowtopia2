document.addEventListener('DOMContentLoaded', function () {
  ...
  let isPresetSolutionLoaded = false;
  let presetSolutionSensorCount = 0;
  let isLeaderboardLockedForCurrentRun = false;
  ...
  let lightningAudioTemplate = null;
  let crunchAudioTimeouts = [];
  let activeCrunchAudios = [];
  let isProModeGuidePending = false;
  let isSignalIntroPending = false;
  let hasCompletedProModeIntro = false;
  let levelTwoGuideStep = '';
  let levelTwoGuideToast = null;
  ...
  const lightningAudioPath = './audio/lightning-strike.wav';
  ...
  const catSkinAssets = {
    ...
    pikachu: {
      name: 'Pikachu',
      map: './image/pikachu-cat.png',
      full: './image/full-pikachu-cat.png'
    }
  };
  const fishSkinAssets = {
    ...
    pikachu: {
      ...
      idle: './image/pikachu-energy1.png',
      eaten: './image/pikachu-energy5.png',
      idleMapSize: '88%',
      eatenMapSize: '86%',
      animationPadding: '4%',
      finalPadding: '8%',
      sound: 'electric',
      effect: 'electric',
      lightningBurstFrames: [
        { src: './image/pikachu-burst1.png', at: 0 },
        { src: './image/pikachu-burst2.png', at: 90 },
        { src: './image/pikachu-burst3.png', at: 210 },
        { src: './image/pikachu-burst4.png', at: 390 },
        { src: './image/pikachu-burst5.png', at: 650 }
      ],
      frames: [
        { src: './image/pikachu-energy2.png', at: 150 },
        { src: './image/pikachu-energy3.png', at: 360 },
        { src: './image/pikachu-energy4.png', at: 680 },
        { src: './image/pikachu-energy5.png', at: 1120, isFinal: true }
      ]
    }
  };
  ...
  const catPlacementHintLevels = new Set([14, 15, 16, 19]);
...
`, 50);
  ...
  function renderThresholdBandLegend() {
    statusBar.innerHTML = `
      <div class="game-coverage-legend threshold-band-legend">
        <div class="game-coverage-title">Threshold</div>
        <div class="game-coverage-item">
          <span class="game-coverage-swatch threshold-band-none"></span>
          <span>&lt; 0.2</span>
        </div>
        <div class="game-coverage-item">
          <span class="game-coverage-swatch threshold-band-1"></span>
          <span>&gt; 0.2</span>
        </div>
        <div class="game-coverage-item">
          <span class="game-coverage-swatch threshold-band-2"></span>
          <span>&gt; 0.3</span>
        </div>
        <div class="game-coverage-item">
          <span class="game-coverage-swatch threshold-band-3"></span>
          <span>&gt; 0.4</span>
        </div>
        <div class="game-coverage-item">
          <span class="game-coverage-swatch threshold-band-4"></span>
          <span>&gt; 0.45</span>
        </div>
        <div class="game-coverage-item">
          <span class="game-coverage-swatch threshold-band-5"></span>
          <span>&gt; 0.55</span>
        </div>
        <div class="game-coverage-item">
          <span class="game-coverage-swatch threshold-band-6"></span>
          <span>&gt; 0.75</span>
        </div>
      </div>
    `;
  ...
  container.className = 'map-grid-wrap';
  ...
  const mapActionSidebar = document.createElement('div');
  mapActionSidebar.className = 'map-action-sidebar';
  const leftControlPanel = document.createElement('div');
  leftControlPanel.className = 'game-control-panel map-control-panel';
  const leftButtonGrid = document.createElement('div');
  leftButtonGrid.className = 'button-row game-button-grid';
  ...
  if (thresholdBtn) {
    leftButtonGrid.appendChild(thresholdBtn);
  }
  if (saveMapBtn) {
  ...
  if (controlPanel) {
    const rightButtonGrid = controlPanel.querySelector('.game-button-grid');
    if (rightButtonGrid) {
      rightButtonGrid.insertBefore(resetBtn, undoBtn || rightButtonGrid.firstChild);
    }
  ...
  function scheduleTooltipHide() {
    ...
    tooltipHideTimeout = setTimeout(hideTooltip, 3000);
  }
  ...
  completionModal.innerHTML = `
    <div class="completion-card">
      ...
      <div class="completion-actions">
        <button id="completion-replay-btn" class="completion-action-button completion-replay-button" type="button">PLAY AGAIN</button>
  ...
  function getTutorialMarkup(key) {
    const terms = getGameTerms();
    const fishSkin = getFishSkin();
    const catSkin = getCatSkin();
    const preyLower = terms.prey.toLowerCase();
    const preyPluralLower = terms.preyPlural.toLowerCase();
    const placeTitle = isPikachuTheme() ? 'Charge The Map With Pikachu' : 'Put A Cat On The Map';
    const coveredAction = isPikachuTheme() ? 'get charged' : 'get eaten';
    const miniFishCell = `<div class="tutorial-mini-cell"><img src="${fishSkin.idle}" alt=""></div>`;
    const miniEatenFishCell = `<div class="tutorial-mini-cell"><img src="${fishSkin.eaten}" alt=""></div>`;
    const hubSkin = isPikachuTheme()
      ? { src: './image/pikachu-hub.png', alt: 'Thunder hub' }
      : { src: './image/pills.png', alt: 'Pills' };

    function renderTutorialFishCell(options = {}) {
      const classes = ['tutorial-mini-cell'];
      if (options.className) classes.push(options.className);
      if (options.tier) classes.push(`tier-${options.tier}`);
      if (options.covered) classes.push('is-covered');
      if (options.blocked) classes.push('is-blocked');
      const src = options.covered ? fishSkin.eaten : fishSkin.idle;
      const size = options.covered
        ? fishSkin.eatenMapSize || fishSkin.idleMapSize || '78%'
        : fishSkin.idleMapSize || '78%';
      return `<div class="${classes.join(' ')}"><img src="${src}" alt="" style="--tutorial-image-size:${size}"></div>`;
    }

    function renderTutorialCatCell(options = {}) {
      const classes = ['tutorial-mini-cell', 'is-cat-cell'];
      if (options.muted) classes.push('is-muted-actor');
      if (options.className) classes.push(options.className);
      return `<div class="${classes.join(' ')}"><img src="${catSkin.map}" alt="" style="--tutorial-image-size:${catSkin.mapSize || '78%'}"></div>`;
    }

    function renderTutorialHubCell() {
      return `<div class="tutorial-mini-cell is-hub-cell"><img src="${hubSkin.src}" alt="${hubSkin.alt}"></div>`;
    }

    const tutorialWallOpenBlockedPositions = new Set([4, 24]);
    const tutorialThresholdLayouts = {
      hard: [
    ...
    function renderTutorialWallBoard(options = {}) {
      const cells = [];
      for (let index = 0; index < 25; index += 1) {
        const x = index % 5;
        const y = Math.floor(index / 5);
        const isActor = x === 1 && y === 2;
        const isWall = options.withWall && x === 2;
        const isBlocked = options.withWall
          ? x > 2
          : tutorialWallOpenBlockedPositions.has(index);
        const isCovered = !isBlocked && !isWall && !isActor;
        if (isActor) {
          cells.push(renderTutorialCatCell());
        } else if (isWall) {
          cells.push('<div class="tutorial-mini-cell wall"></div>');
        } else {
          cells.push(renderTutorialFishCell({
            covered: isCovered,
            blocked: isBlocked,
            className: isBlocked ? 'wall-blocked-target' : ''
          }));
        }
      }
      return `<div class="tutorial-mini-board tutorial-five-board tutorial-dynamic-board tutorial-wall-board">${cells.join('')}</div>`;
    }

    function renderTutorialThresholdBoard(kind) {
      const layout = (tutorialThresholdLayouts[kind] || tutorialThresholdLayouts.hard).flat();
      const cells = layout.map((cellState, index) => {
        if (cellState === 'cat') {
          return renderTutorialCatCell();
        }
        return renderTutorialFishCell({
          tier: cellState,
          covered: cellState === 'green',
          className: index === 12 ? 'is-hard-target' : ''
        });
      });
      return `<div class="tutorial-mini-board tutorial-five-board tutorial-dynamic-board tutorial-threshold-board">${cells.join('')}</div>`;
    }

    function renderTutorialRelayBoard(options = {}) {
      const layout = options.linked
        ? tutorialRelayLayouts.connected
        : tutorialRelayLayouts.disconnected;
      const cells = [];
      for (let index = 0; index < 25; index += 1) {
        if (index === layout.catIndex) {
          cells.push(renderTutorialCatCell({ muted: !layout.linked }));
        } else if (index === layout.hubIndex) {
          cells.push(renderTutorialHubCell());
        } else if (layout.linked) {
          cells.push(renderTutorialFishCell({ covered: true }));
        } else {
          cells.push(renderTutorialFishCell({ blocked: true }));
        }
      }
      const linkLine = layout.linked ? '<span class="tutorial-link-line" aria-hidden="true"></span>' : '';
      return `<div class="tutorial-mini-board tutorial-five-board tutorial-dynamic-board tutorial-relay-board">${cells.join('')}${linkLine}</div>`;
    }
    ...
    if (key === 'place-cat') {
      return `
        <div class="tutorial-card">
          ...
          <p class="tutorial-caption">Tap one tile, then watch nearby ${preyPluralLower} ${coveredAction}.</p>
          <div class="tutorial-scene-grid">
            <div class="tutorial-scene-card">
              <div class="tutorial-scene-label">1. Find a ${preyLower} tile</div>
            ...
            <div class="tutorial-arrow" aria-hidden="true">&rarr;</div>
            <div class="tutorial-scene-card">
              <div class="tutorial-scene-label">3. ${terms.actor} covers nearby ${preyPluralLower}</div>
              <div class="tutorial-mini-board is-result-board">
                ${miniEatenFishCell}
                ${miniEatenFishCell}
                ${miniEatenFishCell}
                ${miniEatenFishCell}
                <div class="tutorial-mini-cell is-cat-cell"><img src="${catSkin.map}" alt=""></div>
                ${miniEatenFishCell}
                ${miniEatenFishCell}
                ${miniEatenFishCell}
                ${miniEatenFishCell}
      ...
      return `
        <div class="tutorial-card tutorial-wall-card">
          <div class="tutorial-title-row">
            ...
            <h2>Walls Stop ${terms.actorPlural}</h2>
          </div>
          <p class="tutorial-caption">If a wall is in the way, ${terms.actor} cannot reach that ${preyLower}.</p>
          <div class="tutorial-wall-grid tutorial-wall-lesson-grid">
            <div class="tutorial-scene-card">
              <div class="tutorial-scene-label">No wall</div>
              ${renderTutorialWallBoard({ withWall: false })}
            </div>
            <div class="tutorial-scene-card">
              <div class="tutorial-scene-label">With wall</div>
              ${renderTutorialWallBoard({ withWall: true })}
            </div>
          </div>
          <button class="tutorial-ok-button" type="button">OK!</button>
        </div>
      ...
      return `
        <div class="tutorial-card tutorial-threshold-card">
          <div class="tutorial-title-row">
            ...
            <h2>Red ${terms.preyPlural} Need Help</h2>
          </div>
          <p class="tutorial-caption">Red ${preyPluralLower} are harder. ${terms.actor} must be closer, or more ${terms.actorPlural} can help.</p>
          <div class="tutorial-threshold-grid">
            <div class="tutorial-scene-card">
              <div class="tutorial-scene-label">1. Red ${preyPluralLower} are harder</div>
              ${renderTutorialThresholdBoard('hard')}
            </div>
            <div class="tutorial-scene-card">
              <div class="tutorial-scene-label">2. Too far away</div>
              ${renderTutorialThresholdBoard('far')}
            </div>
            <div class="tutorial-scene-card">
              <div class="tutorial-scene-label">3. Move closer</div>
              ${renderTutorialThresholdBoard('close')}
            </div>
            <div class="tutorial-scene-card">
              <div class="tutorial-scene-label">4. Add more ${terms.actorPlural}</div>
              ${renderTutorialThresholdBoard('more')}
            </div>
          </div>
          <button class="tutorial-ok-button" type="button">OK!</button>
        </div>
    ...
    if (key === 'relay-link') {
      return `
        <div class="tutorial-card tutorial-relay-card">
          <div class="tutorial-title-row">
            <div class="tutorial-badge">${isPikachuTheme() ? 'Thunder Lesson' : 'Pills Lesson'}</div>
            <h2>Link Or No Link</h2>
          </div>
          <p class="tutorial-caption">No link: ${terms.actor} stays gray. Link: ${terms.actor} can cover ${preyPluralLower}.</p>
          <div class="tutorial-wall-grid tutorial-relay-grid">
            <div class="tutorial-scene-card">
              <div class="tutorial-scene-label">No link</div>
              ${renderTutorialRelayBoard({ linked: false })}
            </div>
            <div class="tutorial-scene-card">
              <div class="tutorial-scene-label">Link to ${isPikachuTheme() ? 'thunder hub' : 'pills'}</div>
              ${renderTutorialRelayBoard({ linked: true })}
            </div>
          </div>
          <button class="tutorial-ok-button" type="button">OK!</button>
        </div>
  ...
  let tutorialHelpButton = null;
  let hasDismissedTutorialOnce = false;
  let shouldContinueAfterSignalIntro = false;
  const missionIdPill = document.querySelector('.mission-id-pill');

  if (missionIdPill && (tutorialMarkup || isProModeTutorial)) {
    tutorialHelpButton = document.createElement('button');
    tutorialHelpButton.className = 'tutorial-help-button';
    tutorialHelpButton.type = 'button';
    tutorialHelpButton.textContent = '?';
    tutorialHelpButton.title = 'Show Tutorial';
    tutorialHelpButton.setAttribute('aria-label', 'Show Tutorial');
    tutorialHelpButton.setAttribute('aria-expanded', tutorialMarkup ? 'true' : 'false');
    missionIdPill.insertAdjacentElement('beforebegin', tutorialHelpButton);
  }
  ...
  function setTutorialOpen(isOpen) {
    ...
    if (isOpen) {
      tutorialModal.classList.remove('is-collapsing');
      tutorialModal.classList.remove('is-expanding');
      tutorialModal.style.removeProperty('--tutorial-collapse-x');
      tutorialModal.style.removeProperty('--tutorial-collapse-y');
      tutorialOkButton?.removeAttribute('disabled');
    }

    tutorialModal.classList.toggle('show', isOpen);
    tutorialHelpButton?.setAttribute('aria-expanded', isOpen ? 'true' : 'false');
  }

  function finishTutorialDismissal() {
    setTutorialOpen(false);

    if (shouldContinueAfterSignalIntro) {
      shouldContinueAfterSignalIntro = false;
      isSignalIntroPending = false;
      if (isLevelTwoProGuideEnabled()) {
    ...
    if (!hasDismissedTutorialOnce) {
      hasDismissedTutorialOnce = true;
      if (isProModeTutorial && !hasCompletedProModeIntro) {
        setProModeGuideActive(true);
        return;
      }
    }
  ...
  function resetLevelTwoInteractionGuide() {
    setProModeGuideActive(false);
    isSignalIntroPending = false;
    shouldContinueAfterSignalIntro = false;
    levelTwoGuideStep = '';
    clearLevelTwoGuideTargets();
    clearLevelTwoSideGuide();
    levelTwoGuideToast?.classList.remove('show');
  }

  function startProModeTutorialFromHelp() {
    if (!isProModeTutorial) {
      return;
    }

    tutorialHelpButton?.setAttribute('aria-expanded', 'true');

    if (isLevelTwoProGuideEnabled()) {
      resetLevelTwoInteractionGuide();
      hasCompletedProModeIntro = false;
      isGameMode = true;
      isThresholdMode = false;
      lastSavedCompletionKey = '';
      isPresetSolutionLoaded = false;
      presetSolutionSensorCount = 0;
      isLeaderboardLockedForCurrentRun = false;
      updateGrid([]);
      updateModeUI();
      resetMissionTimer();
      setProModeGuideActive(true);
  ...
  function dismissTutorial() {
    if (!tutorialModal || tutorialModal.classList.contains('is-collapsing')) {
      return;
    }

    const tutorialCard = tutorialModal.querySelector('.tutorial-card');
    if (!tutorialCard || !tutorialHelpButton) {
      finishTutorialDismissal();
      return;
    }

    const cardRect = tutorialCard.getBoundingClientRect();
    const buttonRect = tutorialHelpButton.getBoundingClientRect();
    const collapseX = buttonRect.left + buttonRect.width / 2 - (cardRect.left + cardRect.width / 2);
    const collapseY = buttonRect.top + buttonRect.height / 2 - (cardRect.top + cardRect.height / 2);

    tutorialModal.style.setProperty('--tutorial-collapse-x', `${collapseX}px`);
    tutorialModal.style.setProperty('--tutorial-collapse-y', `${collapseY}px`);
    tutorialOkButton?.setAttribute('disabled', 'true');
    tutorialModal.classList.add('is-collapsing');

    let hasCompletedCollapse = false;
    const completeCollapse = function () {
      if (hasCompletedCollapse) {
        return;
      }

      hasCompletedCollapse = true;
      tutorialCard.removeEventListener('animationend', completeCollapse);
      tutorialModal.classList.remove('is-collapsing');
      tutorialModal.style.removeProperty('--tutorial-collapse-x');
      tutorialModal.style.removeProperty('--tutorial-collapse-y');
      tutorialOkButton?.removeAttribute('disabled');
    ...
    tutorialCard.addEventListener('animationend', completeCollapse, { once: true });
    window.setTimeout(completeCollapse, 520);
  }

  function expandTutorialFromHelpButton() {
    ...
    window.requestAnimationFrame(() => {
      tutorialModal.classList.add('is-expanding');
    });

    let hasCompletedExpansion = false;
    const completeExpansion = function () {
      if (hasCompletedExpansion) {
        return;
      }

      hasCompletedExpansion = true;
      tutorialCard.removeEventListener('animationend', completeExpansion);
      tutorialModal.classList.remove('is-expanding');
      tutorialModal.style.removeProperty('--tutorial-collapse-x');
      tutorialModal.style.removeProperty('--tutorial-collapse-y');
      tutorialOkButton?.removeAttribute('disabled');
    ...
    tutorialCard.addEventListener('animationend', completeExpansion, { once: true });
    window.setTimeout(completeExpansion, 520);
  }

  function readSkinPreferences() {
    try {
      const saved = JSON.parse(sessionStorage.getItem(skinStorageKey) || '{}');
      return {
  ...
  function isPikachuTheme() {
    return skinPreferences.cat === 'pikachu' || skinPreferences.fish === 'pikachu';
  }

  function getGameTerms() {
    if (!isPikachuTheme()) {
      return {
        actor: 'Cat',
        actorPlural: 'Cats',
        ...
        hub: 'Pills Hub',
        link: 'Cat Link',
        completeTitle: 'Purrfect! Tummies are full!',
        completeMessage: 'All fish are covered!',
        statsLabel: 'CAT'
      };
    }
    ...
    return {
      actor: 'Pikachu',
      actorPlural: 'Pikachu',
      prey: 'Electric Energy',
      preyPlural: 'Electric Energy',
      sense: 'Pika Power',
      need: 'Spark Need',
      hub: 'Thunder Hub',
      link: 'Pika Link',
      completeTitle: 'Pika Pika! Full Charge!',
      completeMessage: 'Every spark is charged and ready!',
      statsLabel: 'PIKACHU'
    };
  }
  ...
  function updateCounters() {
    const terms = getGameTerms();
    setStatLabel(gbestDisplay, isGameMode ? terms.actor : 'Sensors');
  ...
  function updateLegendLabels() {
    ...
    if (labelBoxes[0]) {
      labelBoxes[0].lastChild.textContent = isGameMode ? ` : ${terms.actor}` : ' : Sensor';
    }
    if (labelBoxes[1]) {
      labelBoxes[1].lastChild.textContent = isGameMode ? ` : ${terms.prey}` : ' : Weak Zone';
    }
    if (sensorIcon) {
  ...
  function updateThresholdButtonLabel() {
    if (!thresholdBtn) {
      return;
    }
  ...
  function syncActionButtonLabels() {
    [thresholdBtn, undoBtn, saveMapBtn, quickSolutionBtn, resetBtn].forEach(button => {
      if (!button) {
        return;
      }
  ...
  function updateProToolVisibility() {
    [thresholdBtn, saveMapBtn, quickSolutionBtn].forEach(button => {
      if (button) {
        button.hidden = isGameMode;
        button.style.display = isGameMode ? 'none' : '';
      }
    });
  }
  ...
  function updateCompletionText(completionRate) {
    ...
    if (isGameMode) {
      const progress = Math.max(0, Math.min(100, coverage));
      const progressText = `${progress.toFixed(2)}%`;
      setStatLabel(mapCheck, 'Progress');
      mapCheck.innerHTML = `
        <span class="game-progress-meter" role="progressbar" aria-valuemin="0" aria-valuemax="100" aria-valuenow="${progress.toFixed(0)}" aria-label="Fish eaten progress">
          <span class="game-progress-fill" style="--game-progress-percent: ${progress}%"></span>
        </span>
        <span class="game-progress-value">${progressText}</span>
      `;
      return;
    }
  ...
  function hideCompletionModal() {
    ...
    window.MeowtopiaAccount?.setLeaderboardSpotlight?.(false);
  }

  function closeCompletionAndLockLeaderboard() {
    isLeaderboardLockedForCurrentRun = true;
    hideCompletionModal();
  }

  function setProModeGuideActive(isActive) {
    isProModeGuidePending = isActive;
    document.body.classList.toggle('pro-mode-guide-active', isActive);
    if (modeToggleBtn) {
      modeToggleBtn.classList.toggle('pro-mode-guide-pulse', isActive);
    }
    if (isActive && isLevelTwoProGuideEnabled()) {
      resetMissionTimer();
    }
  }

  function isLevelTwoProGuideEnabled() {
    return isProModeTutorial && getPageLevelNumber() === 2;
  }

  function isLevelTwoGuideBlockingTimer() {
    return isLevelTwoProGuideEnabled() && (
      isProModeGuidePending ||
      isSignalIntroPending ||
  ...
  function setLevelTwoGuideMessage(message) {
    if (!levelTwoGuideToast) {
      levelTwoGuideToast = document.createElement('div');
      levelTwoGuideToast.className = 'level-two-guide-toast';
      document.body.appendChild(levelTwoGuideToast);
    }

    levelTwoGuideToast.textContent = message;
    levelTwoGuideToast.classList.add('show');
  }

  function clearLevelTwoGuideTargets() {
    document.body.classList.remove('level-two-cell-guide-active');
    document.body.classList.remove('level-two-coverage-guide-active');
    getCellAt(5, 5)?.classList.remove('level-two-guide-target-cell');
    thresholdBtn?.classList.remove('level-two-guide-target-button');
  }
  ...
  function showLevelTwoSideGuide(step, title, message, onOk) {
    levelTwoGuideStep = step;
    resetMissionTimer();
    clearLevelTwoGuideTargets();
    clearLevelTwoSideGuide();
    ...
    statusBar?.insertAdjacentHTML('beforeend', `
      <div class="level-two-side-guide-card">
        <div class="level-two-side-guide-title">${title}</div>
        <div class="level-two-side-guide-copy">${message}</div>
        <button class="level-two-side-guide-ok" type="button">OK!</button>
      </div>
    `);
    statusBar?.querySelector('.level-two-side-guide-ok')?.addEventListener('click', onOk, { once: true });
  }

  function finishLevelTwoInteractionGuide() {
    levelTwoGuideStep = 'done';
    clearLevelTwoGuideTargets();
    clearLevelTwoSideGuide();
    tutorialHelpButton?.setAttribute('aria-expanded', 'false');
    setLevelTwoGuideMessage('Start playing now!');
    window.setTimeout(() => {
      levelTwoGuideToast?.classList.remove('show');
    }, 1800);
    startMissionTimer();
  ...
  function showSimpleViewIntro() {
    showLevelTwoSideGuide(
      'simple-intro',
      'Simple View',
      'Simple View uses signal icons to show coverage strength fading outward from the sensor center as distance increases.',
      showCoverageViewButtonGuide
    );
  }

  function showCoverageViewButtonGuide() {
    levelTwoGuideStep = 'coverage';
    resetMissionTimer();
    clearLevelTwoGuideTargets();
    clearLevelTwoSideGuide();
    document.body.classList.add('level-two-coverage-guide-active');
    thresholdBtn?.classList.add('level-two-guide-target-button');
  }

  function showCoverageViewIntro() {
    showLevelTwoSideGuide(
      'coverage-intro',
      'Coverage View',
      'Coverage View paints the map by strength. Warmer colors mean stronger coverage; uncovered areas show the current threshold probability.',
      finishLevelTwoInteractionGuide
    );
  }

  function showLevelTwoCellGuide() {
    levelTwoGuideStep = 'cell';
    resetMissionTimer();
    clearLevelTwoGuideTargets();
    clearLevelTwoSideGuide();
    document.body.classList.add('level-two-cell-guide-active');
    const targetCell = getCellAt(5, 5);
    targetCell?.classList.add('level-two-guide-target-cell');
  }

  function showProModeSignalIntro() {
    if (hasCompletedProModeIntro || !isProModeTutorial) {
      return;
    }

    isSignalIntroPending = true;
    resetMissionTimer();
    hasCompletedProModeIntro = true;
    shouldContinueAfterSignalIntro = true;
    ...
    tutorialModal.className = 'tutorial-modal pro-signal-modal show';
    tutorialModal.innerHTML = `
      <div class="tutorial-card signal-intro-card">
        <div class="tutorial-title-row">
          <div class="tutorial-badge">Pro Mode</div>
          <h2>Read The Signal</h2>
        </div>
        <p class="tutorial-caption">Signal shows how strongly each cat covers nearby fish. Higher signal means stronger coverage.</p>
        <div class="signal-intro-preview">
          <div class="signal-intro-board">
            <div class="signal-intro-cell level-1">0.2</div>
            <div class="signal-intro-cell level-2">0.4</div>
            <div class="signal-intro-cell level-3">0.6</div>
            <div class="signal-intro-cell level-4">0.8</div>
            <div class="signal-intro-cell level-5">1.0</div>
          </div>
          <div class="signal-intro-copy">
            <strong>Use the Signal box</strong>
            <span>Compare colors and values to find weak zones before placing your next cat.</span>
          </div>
        </div>
        <button class="tutorial-ok-button" type="button">OK!</button>
      </div>
    ...
    tutorialOkButton = tutorialModal.querySelector('.tutorial-ok-button');
    tutorialHelpButton?.setAttribute('aria-expanded', 'true');
    tutorialOkButton?.addEventListener('click', dismissTutorial);
  }
  ...
  function getUnlockedPlacementLevel() {
    ...
    const unlockedLevel = Math.max(
      Number.isFinite(currentKeyValue) ? currentKeyValue : 1,
      Number.isFinite(legacyKeyValue) ? legacyKeyValue : 1
    );
    return Math.min(Math.max(unlockedLevel, 1), 19);
  }

  function getNextLevelNavLink() {
    return document.querySelector('.level-nav .level-nav-next');
  }

  function setLevelNavLinkDisabled(link, isDisabled) {
    if (!link) {
      return;
    }

    link.classList.toggle('level-nav-disabled', isDisabled);
    if (isDisabled) {
  ...
  function lockNextLevelNav() {
    setLevelNavLinkDisabled(getNextLevelNavLink(), true);
  }

  function refreshNextLevelNavLock() {
    const link = getNextLevelNavLink();
    ...
    const nextLevel = getPageLevelNumber() + 1;
    setLevelNavLinkDisabled(link, nextLevel > getUnlockedPlacementLevel());
  }

  function unlockNextLevelNav() {
    const link = getNextLevelNavLink();
    if (!link?.getAttribute('href')) {
      return;
    }

    setLevelNavLinkDisabled(link, false);
  }
  ...
  function startMissionTimer() {
    if (isLevelTwoGuideBlockingTimer()) {
      resetMissionTimer();
      return;
    }
  ...
  function clearFishAnimations() {
    ...
    const electricLayers = clickableBox.querySelectorAll('.electric-burst-layer');
    electricLayers.forEach(layer => layer.remove());
    const electricCells = clickableBox.querySelectorAll('.grid-cell.electric-bursting');
    electricCells.forEach(cell => cell.classList.remove('electric-bursting'));
    const electricEntities = clickableBox.querySelectorAll('.cell-entity.electric-charging');
    electricEntities.forEach(entity => entity.classList.remove('electric-charging'));
  }
  ...
  function getLightningAudioTemplate() {
    if (typeof Audio === 'undefined') {
      return null;
    }

    if (!lightningAudioTemplate) {
      lightningAudioTemplate = new Audio(lightningAudioPath);
      lightningAudioTemplate.preload = 'auto';
      lightningAudioTemplate.load();
    }

    return lightningAudioTemplate;
  }
  ...
  function playElectricCrackleBurst(ctx, startTime, duration = 0.22, intensity = 1) {
    const bufferSize = Math.max(1, Math.floor(ctx.sampleRate * duration));
    const noiseBuffer = ctx.createBuffer(1, bufferSize, ctx.sampleRate);
    const channelData = noiseBuffer.getChannelData(0);
    ...
    for (let i = 0; i < bufferSize; i++) {
      ...
      const staticNoise = (Math.random() * 2 - 1) * 0.9;
      const arcSnap = Math.random() > 0.76 ? (Math.random() * 2 - 1) * 3.4 : 0;
      const needle = Math.random() > 0.955 ? (Math.random() * 2 - 1) * 5.2 : 0;
      channelData[i] = (staticNoise + arcSnap + needle) * fade;
    }
    ...
    const highpass = ctx.createBiquadFilter();
    highpass.type = 'highpass';
    highpass.frequency.setValueAtTime(360, startTime);

    const bandpass = ctx.createBiquadFilter();
    bandpass.type = 'bandpass';
    bandpass.frequency.setValueAtTime(1700 + Math.random() * 1900, startTime);
    bandpass.Q.setValueAtTime(4.8, startTime);

    const gainNode = ctx.createGain();
    gainNode.gain.setValueAtTime(0.0001, startTime);
    gainNode.gain.exponentialRampToValueAtTime(0.32 * intensity, startTime + 0.004);
    gainNode.gain.exponentialRampToValueAtTime(0.0001, startTime + duration);
  ...
  function playElectricThunderTail(ctx, startTime, duration = 2.45) {
    const merger = ctx.createGain();
    const lowpass = ctx.createBiquadFilter();
    const subOscillator = ctx.createOscillator();
    const rollOscillator = ctx.createOscillator();
    const gainNode = ctx.createGain();

    subOscillator.type = 'sine';
    subOscillator.frequency.setValueAtTime(46, startTime);
    subOscillator.frequency.exponentialRampToValueAtTime(27, startTime + duration);

    rollOscillator.type = 'sawtooth';
    rollOscillator.frequency.setValueAtTime(92, startTime);
    rollOscillator.frequency.exponentialRampToValueAtTime(42, startTime + duration);

    lowpass.type = 'lowpass';
    lowpass.frequency.setValueAtTime(520, startTime);
    lowpass.frequency.exponentialRampToValueAtTime(105, startTime + duration);

    gainNode.gain.setValueAtTime(0.0001, startTime);
    gainNode.gain.exponentialRampToValueAtTime(0.2, startTime + 0.08);
    gainNode.gain.exponentialRampToValueAtTime(0.07, startTime + 0.92);
    gainNode.gain.exponentialRampToValueAtTime(0.0001, startTime + duration);

    subOscillator.connect(merger);
    rollOscillator.connect(merger);
    merger.connect(lowpass);
    lowpass.connect(gainNode);
    gainNode.connect(ctx.destination);
    subOscillator.start(startTime);
    rollOscillator.start(startTime);
    subOscillator.stop(startTime + duration);
    rollOscillator.stop(startTime + duration);
  }

  function playSyntheticElectricSequence(durationMs = 1300) {
    const ctx = getAudioContext();
    if (!ctx) {
      return;
    }
    ...
    const startTime = ctx.currentTime + 0.02;
    playElectricCrackleBurst(ctx, startTime, 0.24, 1.32);
    playElectricThunderTail(ctx, startTime + 0.04, Math.min(3.1, Math.max(2.4, durationMs / 1000 + 1.35)));

    [130, 290, 540, 960, 1500].forEach((time, index) => {
      const timeoutId = window.setTimeout(() => {
        playElectricCrackleBurst(ctx, ctx.currentTime + 0.01, 0.08 + index * 0.018, 0.72 + index * 0.05);
      }, Math.min(durationMs + 760, time));
      crunchAudioTimeouts.push(timeoutId);
    });
  }

  function playElectricSequence(durationMs = 1300) {
    const template = getLightningAudioTemplate();
    const clipDurationMs = Math.min(3400, Math.max(3000, durationMs + 1850));

    if (!template) {
      playSyntheticElectricSequence(durationMs);
      return;
    }

    const lightningAudio = template.cloneNode(true);
    lightningAudio.preload = 'auto';
    lightningAudio.currentTime = 0;
    lightningAudio.volume = 0.86;
    lightningAudio.playbackRate = 1;
    activeCrunchAudios.push(lightningAudio);

    const playPromise = lightningAudio.play();
    if (playPromise && typeof playPromise.catch === 'function') {
      playPromise.catch(() => {
        lightningAudio.pause();
        lightningAudio.currentTime = 0;
        removeTrackedCrunchAudio(lightningAudio);
        playSyntheticElectricSequence(durationMs);
      });
    }
  ...
  function playEatSoundSequence(durationMs = 2000) {
    if (getFishSkin().sound === 'electric') {
      playElectricSequence(durationMs);
      return;
    }
  ...
  function runFishEatAnimation(entityLayer) {
    ...
    if (fishSkin.effect === 'electric') {
      runElectricChargeAnimation(entityLayer, fishSkin);
      return;
    }
    ...
    entityLayer.style.padding = fishSkin.animationPadding || '11%';
    ...
    frames.forEach(frame => {
      const timeoutId = window.setTimeout(() => {
        ...
        if (frame.isFinal) {
          ...
          entityLayer.style.padding = fishSkin.finalPadding || '12%';
        }
      }, frame.at);
  ...
  function runElectricChargeAnimation(entityLayer, fishSkin = getFishSkin()) {
    const cell = entityLayer.closest('.grid-cell');
    const frames = fishSkin.frames || [];
    const burstFrames = fishSkin.lightningBurstFrames || [];
    const burstLayer = document.createElement('img');

    cell?.querySelectorAll('.electric-burst-layer').forEach(layer => layer.remove());
    cell?.classList.add('electric-bursting');

    entityLayer.classList.add('electric-charging');
    entityLayer.src = fishSkin.idle;
    entityLayer.style.display = 'block';
    entityLayer.style.padding = fishSkin.animationPadding || '4%';
    entityLayer.style.boxSizing = 'border-box';

    burstLayer.className = 'electric-burst-layer';
    burstLayer.alt = '';
    burstLayer.draggable = false;
    burstLayer.src = burstFrames[0]?.src || './image/pikachu-burst1.png';
    cell?.appendChild(burstLayer);

    burstFrames.forEach(frame => {
      const timeoutId = window.setTimeout(() => {
        burstLayer.src = frame.src;
      }, frame.at);
      fishAnimationTimeouts.push(timeoutId);
    });
    ...
    frames.forEach(frame => {
      const timeoutId = window.setTimeout(() => {
        entityLayer.src = frame.src;
        if (frame.isFinal) {
          entityLayer.classList.remove('electric-charging');
          cell?.classList.remove('electric-bursting');
          entityLayer.style.padding = fishSkin.finalPadding || '8%';
          burstLayer.classList.add('is-fading');
          const removeTimeoutId = window.setTimeout(() => {
            burstLayer.remove();
          }, 360);
          fishAnimationTimeouts.push(removeTimeoutId);
        }
      }, frame.at);
      fishAnimationTimeouts.push(timeoutId);
    });
  ...
  function showCompletionModal() {
    ...
    if (completionTitle) {
      completionTitle.textContent = isGameMode ? getGameTerms().completeTitle : 'Mission Complete!';
    }
    if (completionMessage) {
      completionMessage.textContent = isGameMode ? getGameTerms().completeMessage : 'Coverage target reached!';
    }
    if (completionStats) {
      completionStats.textContent = `${getGameTerms().statsLabel}: ${sensorCount} | TIME: ${formatSeconds(finalElapsedSeconds)}`;
    }
    ...
    const allowLeaderboard = (!isPresetSolutionLoaded || isReducedPresetSolution()) && !isLeaderboardLockedForCurrentRun;
    completionModal.classList.add('show');
    window.MeowtopiaAccount?.setLeaderboardSpotlight?.(true);
    window.MeowtopiaAccount?.openLeaderboard?.({
      showChampion: false,
      showLoading: allowLeaderboard && isFixedLevelMapSelected(),
      showChampionAfterRender: allowLeaderboard && isFixedLevelMapSelected()
    });
    Promise.resolve(saveCompletionRecord({ allowLeaderboard }));
  }
  ...
  function isReducedPresetSolution() {
    return Boolean(isPresetSolutionLoaded && presetSolutionSensorCount > 0 && sensorCount < presetSolutionSensorCount);
  }

  function saveCompletionRecord(options = {}) {
    const allowLeaderboard = options.allowLeaderboard ?? ((!isPresetSolutionLoaded || isReducedPresetSolution()) && !isLeaderboardLockedForCurrentRun);
    if (!allowLeaderboard) {
      return Promise.resolve(false);
    }

    if (!isFixedLevelMapSelected()) {
      return Promise.resolve(false);
    }
    ...
    if (!window.MeowtopiaAccount) {
      return new Promise(resolve => {
        setTimeout(() => {
          resolve(saveCompletionRecord({ allowLeaderboard }));
        }, 500);
      });
    }
    ...
    return window.MeowtopiaAccount?.savePlacementRecord({
      steps: sensorCount,
      time: isReducedPresetSolution() ? '' : getMissionElapsedSeconds(),
      timeRankable: !isReducedPresetSolution(),
  ...
  function updateModeUI() {
    ...
    const isProColorView = !isGameMode && isThresholdMode;
    document.body.classList.toggle('cat-game-mode', isGameMode);
    document.body.classList.toggle('pikachu-theme', isGameMode && isPikachuTheme());
    document.body.classList.toggle('coverage-view-active', isProColorView);
    renderViewLegend();
    updateCounters();
    updateLegendLabels();
    updateModeButton();
    updateThresholdButtonLabel();
    updateProToolVisibility();
  ...
  function isFixedLevelMapSelected() {
    const fixedSolution = parseInt(document.body.dataset.solution || '', 10);
    return !Number.isFinite(fixedSolution) || fixedSolution === getSelectedLevel();
  }
  ...
  function shouldShowCatPlacementHints(level = getSelectedLevel()) {
    return isGameMode && catPlacementHintLevels.has(level);
  }
  ...
  function buildPikachuLightningPath(length, cellSize) {
    const segmentCount = Math.max(2, Math.min(7, Math.round(length / Math.max(18, cellSize * 0.52))));
    const amplitude = Math.max(8, Math.min(18, cellSize * 0.24));
    const points = [{ x: 0, y: 0 }];

    for (let i = 1; i < segmentCount; i++) {
      const progress = i / segmentCount;
      const alternate = i % 2 === 0 ? -1 : 1;
      const bite = i % 3 === 0 ? 0.55 : 1;
      points.push({
        x: length * progress,
        y: alternate * amplitude * bite
      });
    }
  ...
  function addPikachuLightningSegments(line, length, cellSize) {
    line.classList.add('pikachu-lightning-path');
    const points = buildPikachuLightningPath(length, cellSize);

    points.slice(0, -1).forEach((point, index) => {
      const nextPoint = points[index + 1];
      const deltaX = nextPoint.x - point.x;
      const deltaY = nextPoint.y - point.y;
      const segmentLength = Math.sqrt(Math.pow(deltaX, 2) + Math.pow(deltaY, 2));
      const segmentAngle = Math.atan2(deltaY, deltaX) * (180 / Math.PI);
      const segment = document.createElement('span');
      segment.className = 'pikachu-lightning-segment';
      segment.style.left = `${point.x}px`;
      segment.style.top = `calc(50% + ${point.y}px)`;
      segment.style.width = `${segmentLength}px`;
      segment.style.setProperty('--pikachu-segment-rotate', `${segmentAngle}deg`);
      segment.style.setProperty('--pikachu-segment-delay', `${index * 0.055}s`);
      line.appendChild(segment);
    });

    points.slice(1, -1).forEach((point, index) => {
      const spark = document.createElement('span');
      spark.className = 'pikachu-lightning-spark';
      spark.style.left = `${point.x}px`;
      spark.style.top = `calc(50% + ${point.y}px)`;
      spark.style.setProperty('--pikachu-spark-delay', `${index * 0.07}s`);
  ...
  function connectSensors() {
    ...
    connectivityData.edges.forEach(edge => {
      ...
      line.style.transform = `translateY(var(--connection-line-center-offset, 0)) rotate(${angle}deg)`;

      if (isGameMode && isPikachuTheme()) {
        addPikachuLightningSegments(line, length, cellSize);
      }
  ...
  function mixHexColors(startColor, endColor, ratio) {
    ...
    const mixed = startRgb.map((channel, index) => {
      return Math.round(channel + (endRgb[index] - channel) * clampRatio);
    });
    return `rgb(${mixed[0]}, ${mixed[1]}, ${mixed[2]})`;
  }

  function getGameTerrainColorFromThreshold(cellThreshold, isSatisfied, probability = 0) {
    if (isSatisfied) {
    ...
    let baseColor = '#fff1a8';
    if (cellThreshold >= 0.55) {
      baseColor = '#ff8da1';
    } else if (cellThreshold >= 0.42) {
      baseColor = '#ffb16d';
    } else if (cellThreshold >= 0.3) {
      baseColor = '#ffd86f';
    }
    ...
    const progress = cellThreshold > 0 ? probability / cellThreshold : 0;
    return mixHexColors(baseColor, '#fff1a8', progress);
  }
  ...
  function getThresholdBandColor(value) {
    if (value < 0.2) {
      return '#f8fbff';
    }
    if (value > 0.75) {
  ...
  function updateNearCompleteUncoveredHints(grid, completionRate) {
    ...
    Array.from(grid.childNodes)
      ...
      .forEach(cell => {
        const isUncoveredPlayableCell =
          cell.classList.contains('dissatisfy') &&
  ...
  function updateCatPlacementHints(grid) {
    ...
    const hintClasses = [
      'cat-placeable-cell',
      'cat-placeable-edge-top',
      'cat-placeable-edge-right',
      'cat-placeable-edge-bottom',
      'cat-placeable-edge-left'
    ];

    cells.forEach(cell => cell.classList.remove(...hintClasses));

    if (!shouldShowCatPlacementHints(level)) {
      return;
    }

    const sourceNodes = [
      ...getRelayNodesForLevel(level),
      ...sensors.filter(sensor => activeSensorKeys.has(`${sensor.x},${sensor.y}`))
    ];
    ...
    const connectDistance = getConnectDistanceForLevel(level);
    ...
    cells.forEach((cell, index) => {
      ...
      const isPlaceableNow = sourceNodes.some(node => {
        const distance = Math.sqrt((node.x - x) ** 2 + (node.y - y) ** 2);
        return distance <= connectDistance && hasLineOfSight(node, x, y);
      });

      if (isPlaceableNow) {
        placeableKeys.add(`${x},${y}`);
      }
    });

    const solidKeys = new Set(placeableKeys);
    sourceNodes.forEach(node => solidKeys.add(`${node.x},${node.y}`));

    const exteriorKeys = new Set();
    const queue = [{ x: -1, y: -1 }];
    const paddedMin = -1;
    const paddedMax = gridSize;
    const addExteriorNode = function (x, y) {
      if (x < paddedMin || x > paddedMax || y < paddedMin || y > paddedMax) {
        return;
      }

      const key = `${x},${y}`;
      if (exteriorKeys.has(key) || solidKeys.has(key)) {
        return;
      }
    ...
    exteriorKeys.add('-1,-1');
    for (let index = 0; index < queue.length; index++) {
      const node = queue[index];
      addExteriorNode(node.x, node.y - 1);
      addExteriorNode(node.x + 1, node.y);
      addExteriorNode(node.x, node.y + 1);
      addExteriorNode(node.x - 1, node.y);
    }
    ...
    cells.forEach((cell, index) => {
      ...
      const hasTopEdge = exteriorKeys.has(`${x},${y - 1}`);
      const hasRightEdge = exteriorKeys.has(`${x + 1},${y}`);
      const hasBottomEdge = exteriorKeys.has(`${x},${y + 1}`);
      const hasLeftEdge = exteriorKeys.has(`${x - 1},${y}`);

      if (!hasTopEdge && !hasRightEdge && !hasBottomEdge && !hasLeftEdge) {
        return;
      }

      cell.classList.add('cat-placeable-cell');
      if (hasTopEdge) {
        cell.classList.add('cat-placeable-edge-top');
      }
      if (hasRightEdge) {
        cell.classList.add('cat-placeable-edge-right');
      }
      if (hasBottomEdge) {
        cell.classList.add('cat-placeable-edge-bottom');
      }
      if (hasLeftEdge) {
        cell.classList.add('cat-placeable-edge-left');
      }
    });
  }
  ...
  function applyProbabilitiesToGrid(probabilities, grid) {
    ...
    cells.forEach((cell, index) => {
      ...
      if (cell.classList.contains('relay-node')) {
        ...
        setEntityImage(isGameMode ? (isPikachuTheme() ? './image/pikachu-hub.png' : './image/pills.png') : './image/sensor.png', '72%');
      ...
      const variableThresholdTerrainColor = getGameTerrainColorFromThreshold(cellThreshold, isSatisfied, probability);
      ...
      if (isSatisfied) {
        if (isGameMode) {
          ...
          if (shouldAnimateFish && wasDissatisfied) {
            ...
            const fishSkin = getFishSkin();
            setEntityImage(fishSkin.eaten, fishSkin.eatenMapSize || '76%');
          }
          setRelayBadge('');
        } else {
          const signalImage = getProfessionalSignalImage(probability);
          const defaultProColor = hasVariableThresholdMap()
            ? getProfessionalThresholdColor(cellThreshold)
            : '#eef5fb';
          cell.style.backgroundColor = isThresholdMode
            ? getThresholdBandColor(probability)
            : defaultProColor;
          cell.style.backgroundImage = '';
          cell.style.backgroundSize = '';
          cell.style.backgroundPosition = '';
        ...
        if (isGameMode) {
          ...
          cell.style.backgroundRepeat = '';
          const fishSkin = getFishSkin();
          setEntityImage(fishSkin.idle, fishSkin.idleMapSize || '78%');
          setRelayBadge('');
        } else {
          const signalImage = getProfessionalSignalImage(probability);
          const defaultProColor = hasVariableThresholdMap()
            ...
            : '#f8fbff';
          cell.style.backgroundColor = isThresholdMode
            ? getThresholdBandColor(cellThreshold)
            : defaultProColor;
          cell.style.backgroundImage = '';
          cell.style.backgroundSize = '';
          cell.style.backgroundPosition = '';
  ...
  function calculateDetectionProbability(size, sensorList, grid) {
    ...
    updateNearCompleteUncoveredHints(grid, completionRate);
  ...
  function renderGrid(size, layout = []) {
    ...
    for (let i = 0; i < size * size; i++) {
      ...
      cell.addEventListener('click', function () {
        if (isProModeGuidePending || isSignalIntroPending) {
          return;
        }
        if (
          levelTwoGuideStep === 'coverage-intro' ||
        ...
        if (levelTwoGuideStep === 'cell' && (x !== 4 || y !== 4)) {
          return;
        }
        saveMapState();

        if (cell.classList.contains('sensor')) {
          if (isPresetSolutionLoaded) {
            lastSavedCompletionKey = '';
          }
          shouldAnimateFishOnNextUpdate = false;
          cell.classList.remove('sensor');
      ...
      cell.addEventListener('mousemove', function (event) {
        ...
        if (cell.classList.contains('relay-node')) {
          ...
          tooltip.innerHTML = isGameMode
            ? `Row ${rowNumber}, Column ${columnNumber}<br>${terms.hub}<br>${terms.link}: ${getConnectDistanceForLevel()} tiles`
        ...
        const terms = getGameTerms();
        tooltip.innerHTML = isGameMode
          ? `Row ${rowNumber}, Column ${columnNumber}<br>${terms.sense}: ${(probability * 100).toFixed(1)}%<br>${terms.need}: ${(cellThreshold * 100).toFixed(0)}%`
  ...
  completionClose?.addEventListener('click', closeCompletionAndLockLeaderboard);
  completionReplayBtn?.addEventListener('click', replayCurrentLevel);
  completionLevelsBtn?.addEventListener('click', function () {
    window.location.href = 'index.html';
  });
  completionModal.addEventListener('click', function (event) {
    if (event.target === completionModal) {
      closeCompletionAndLockLeaderboard();
    }
  });
  tutorialOkButton?.addEventListener('click', dismissTutorial);
  tutorialHelpButton?.addEventListener('click', function () {
    if (isLevelTwoProGuideEnabled() || (!tutorialModal && isProModeTutorial)) {
      startProModeTutorialFromHelp();
      return;
    }
    expandTutorialFromHelpButton();
  });
  ...
  thresholdBtn?.addEventListener('click', function () {
    ...
    if (levelTwoGuideStep && levelTwoGuideStep !== 'coverage' && levelTwoGuideStep !== 'done') {
      return;
    }
    ...
    shouldAnimateFishOnNextUpdate = false;
    updateThresholdButtonLabel();
    document.body.classList.toggle('coverage-view-active', isThresholdMode);
    renderViewLegend();
    applyProbabilitiesToGrid(probabilitiesCache, clickableBox);
    if (levelTwoGuideStep === 'coverage') {
      showCoverageViewIntro();
    }
  });
  ...
  resetBtn.addEventListener('click', function () {
    if (isProModeGuidePending || isSignalIntroPending || isLevelTwoGuideBlockingTimer()) {
      return;
    }
    const elapsedBeforeReset = getMissionElapsedSeconds();
    ...
    if (!tutorialModal?.classList.contains('show')) {
      if (!missionTimerInterval) {
        missionStartedAt = Date.now() - elapsedBeforeReset * 1000;
        finalElapsedSeconds = 0;
      }
      startMissionTimer();
    }
  ...
  modeToggleBtn?.addEventListener('click', function () {
    ...
    if (shouldShowSignalIntro) {
      showProModeSignalIntro();
    }
  });
  ...
  solutionDropdown?.addEventListener('change', function () {
    ...
    presetSolutionSensorCount = 0;
    isLeaderboardLockedForCurrentRun = false;
    refreshNextLevelNavLock();
    updateConnectModeForLevel(getSelectedLevel());
    renderGrid(getSelectedGridSize(), []);
    if (!tutorialModal?.classList.contains('show') && !isLevelTwoGuideBlockingTimer()) {
      startMissionTimer();
    }
  });
  ...
  updateUndoButtonState();
  renderGrid(getSelectedGridSize(), []);
  document.body.classList.add('map-model-ready');
  if (!tutorialModal) {
    startMissionTimer();
  }
});
